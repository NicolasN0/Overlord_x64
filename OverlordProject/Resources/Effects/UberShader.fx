...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
    ...
    return gColorSpecular * specValue;
}
...
float3 CalculateSpecular(float3 viewDirection, float3 normal, float2 texCoord)
{
    ...
    {
        spec += saturate(CalculateSpecularBlinn(viewDirection, normal, texCoord));
    }
		
    if (gUseSpecularPhong)
    {
        spec += saturate(CalculateSpecularPhong(viewDirection, normal, texCoord));
    }
    ...
    return saturate(spec * gTextureSpecularIntensity.Sample(gTextureSampler, texCoord).r);
}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
    ...
    float fresnel = fresnelVal * fresnelMask;
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
    if (gUseEnvironmentMapping)
    {
        ...
        float3 cubeMapColor = gCubeEnvironment.Sample(gTextureSampler, reflectedVector) * gReflectionStrength;
        return cubeMapColor;
    }
    return float3(0, 0, 0);
}
