...
void Character::Update(const SceneContext& sceneContext)
{
	if (m_pCameraComponent->IsActive())
	{
		...
		XMVECTOR velocityVec = XMLoadFloat3(&m_CurrentDirection);
		XMVECTOR horVelocity = velocityVec * m_MoveSpeed;
		XMFLOAT3 velocity{};
		XMStoreFloat3(&velocity, horVelocity);
		...
		XMStoreFloat3(&displacement, XMLoadFloat3(&m_TotalVelocity) * sceneContext.pGameTime->GetElapsed());
